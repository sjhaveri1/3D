...
if sum(shortTargets(:,1)) == 0
    ...
    for i=1:size(extraTargets,1)
        points(i,:) = coords(extraTargets(i,1),:);
    end
    midpt = avgMidpoint(points);
    branchPoints = [size(points,1),midpt];
    steinerLeaves{1} = extraTargets(:,1);
    bi = branchPoints(1,2:4);
    ...
    if size(shortTargets,1) == 1
        pt2 = coords(shortTargets(1,1),:);
